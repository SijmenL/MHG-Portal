    ...
    <div class="container col-md-11">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="{{ route('lessons.environment.lesson', $lesson->id) }}">{{ $lesson->title }}</a></li>
        ...
        <div class="mt-1 bg-white p-2 rounded d-flex flex-row justify-content-between">
            ...
            <div class="d-flex flex-row">
                ...
                @endif
                @if((auth()->user()->roles->contains('role', 'Praktijkbegeleider') || auth()->user()->roles->contains('role', 'Administratie') || auth()->user()->roles->contains('role', 'Bestuur')|| auth()->user()->roles->contains('role', 'Ouderraad')))
                    <a class="delete-button btn d-flex align-items-center"
                       ...
                       @endif
                       data-link="{{ route('lessons.environment.lesson.post.delete', [$lesson->id, $post->id]) }}"><span
        ...
        <div id="comments" class="p-3 comment-section">
            ...
            <div class="bg-light rounded-2 p-3">
                <div class="container">
                    <form method="POST" action="{{ route('lessons.environment.lesson.comment-post', [$lesson->id, $post->id]) }}">
                        @csrf
                            ...
                            <div class="w-100 p-1" style=" width: 100%; overflow: auto">
                                ...
                                <div class="content mt-1">
                                        ...
                                        @if($comment->user->id === Auth::id())
                                            <a
                                                class="edit-button btn d-flex align-items-center edit"><span
                                                    class="material-symbols-rounded">edit</span></a>
                                        @endif
                                        @if($comment->user->id === Auth::id() || (auth()->user()->roles->contains('role', 'Loodsen Stamoudste') || auth()->user()->roles->contains('role', 'Administratie') || auth()->user()->roles->contains('role', 'Bestuur')|| auth()->user()->roles->contains('role', 'Ouderraad')))
                                            <a class="delete-button btn d-flex align-items-center"
                                               data-id="{{ $post->id }}"
                                               ...
                                               data-link="{{ route('lessons.environment.lesson.comment.delete', ['lessonId' => $lesson->id, 'id' => $comment->id, 'postId' => $post->id]) }}">
                                    ...
                                    <form method="POST" class="comment-form" data-post-id="{{ $post->id }}"
                                          ...
                                          action="{{ route('lessons.environment.lesson.reaction-post', ['lessonId' => $lesson->id, 'id' => $post->id, 'commentId' => $comment->id]) }}">
                                        @csrf
                                                    ...
                                                    @if($reaction->user->id === Auth::id())
                                                        <a
                                                            class="edit-button btn d-flex align-items-center edit"><span
                                                                class="material-symbols-rounded">edit</span></a>
                                                    @endif
                                                    @if($reaction->user->id === Auth::id() || (auth()->user()->roles->contains('role', 'Loodsen Stamoudste') || auth()->user()->roles->contains('role', 'Administratie') || auth()->user()->roles->contains('role', 'Bestuur')|| auth()->user()->roles->contains('role', 'Ouderraad')))
                                                        <a class="delete-button btn d-flex align-items-center"
                                                           data-id="{{ $post->id }}"
                                                           ...
                                                           data-link="{{ route('lessons.environment.lesson.comment.delete', ['lessonId' => $lesson->id, 'id' => $reaction->id, 'postId' => $post->id]) }}">
                                                            <span class="material-symbols-rounded">delete</span>
                                                        </a>
