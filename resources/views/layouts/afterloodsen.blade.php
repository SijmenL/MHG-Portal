...
<head>
    ...
    <!-- Scripts -->
    @vite(['resources/sass/app.scss', 'resources/css/app.css', 'resources/js/app.js', 'resources/js/texteditor.js', 'resources/css/texteditor.css'])
</head>
<body>
<div id="app">
    <div>
                        ...
                        <ul class="navbar-nav align-items-start justify-content-start justify-content-md-start" id="navbar">
                            <li class="nav-item">
                                <a class="nav-link d-flex flex-row gap-1 justify-content-center align-items-center white-text"
                                   href="{{ route('afterloodsen') }}"><img alt="afterloodsen" class="navbar-icon" src="{{ asset('img/icons/after_loodsen.png') }}">Home</a>
                            </li>
                            @if(auth()->user() && (auth()->user()->roles->contains('role', 'Afterloodsen Leiding') || auth()->user()->roles->contains('role', 'Administratie') || auth()->user()->roles->contains('role', 'Bestuur')|| auth()->user()->roles->contains('role', 'Ouderraad')))
                                <li class="nav-item">
                                    <a class="nav-link d-flex flex-row gap-1 justify-content-center align-items-center white-text"
                                       href="{{ route('afterloodsen.groep') }}"><span
                                            class="material-symbols-rounded">groups_2</span>Leden</a>
                                </li>
                            @endif
                            <li class="nav-item">
                                <a class="nav-link d-flex flex-row gap-1 justify-content-center align-items-center white-text"
                                   href="{{ route('afterloodsen.leiding') }}"><span
                                        class="material-symbols-rounded">supervisor_account</span>Organisatie</a>
                            </li>
                            {{--                            <li class="nav-item">--}}
                            {{--                                <a class="nav-link d-flex flex-row gap-1 justify-content-center align-items-center white-text"--}}
