    ...
    <div class="container col-md-11">
        ...
        <div class="mt-1 bg-white p-2 rounded d-flex flex-row justify-content-between">
            ...
            <div class="d-flex flex-row">
                ...
                @endif
                @if((auth()->user()->roles->contains('role', 'Zeeverkenners Leiding') || auth()->user()->roles->contains('role', 'Administratie') || auth()->user()->roles->contains('role', 'Bestuur')|| auth()->user()->roles->contains('role', 'Ouderraad')))
                    <a class="delete-button btn d-flex align-items-center"
                       ...
                       @endif
                       data-link="{{ route('zeeverkenners.post.delete', $post->id) }}"><span
                            ...
                            <div class="w-100 p-1" style=" width: 100%; overflow: auto">
                                ...
                                <div class="content mt-1">
                                        ...
                                        @if($comment->user->id === Auth::id())
                                            <a
                                                class="edit-button btn d-flex align-items-center edit"><span
                                                    class="material-symbols-rounded">edit</span></a>
                                        @endif
                                        @if($comment->user->id === Auth::id() || (auth()->user()->roles->contains('role', 'Zeeverkenners Leiding') || auth()->user()->roles->contains('role', 'Administratie') || auth()->user()->roles->contains('role', 'Bestuur')|| auth()->user()->roles->contains('role', 'Ouderraad')))
                                            <a class="delete-button btn d-flex align-items-center"
                                               data-id="{{ $post->id }}"
                                               ...
                                               data-link="{{ route('zeeverkenners.comment.delete', ['id' => $comment->id, 'postId' => $post->id]) }}">
                                    ...
                                    <form method="POST" class="comment-form" data-post-id="{{ $post->id }}"
                                          ...
                                          action="{{ route('zeeverkenners.reaction-post', ['id' => $post->id, 'commentId' => $comment->id]) }}">
                                        @csrf
                                        <div class="d-flex flex-row gap-2 align-items-end comment-input">
                                            <div class="text-input w-100" id="text-input" style="min-height: 75px"
                                                    ...
                                                    @if($reaction->user->id === Auth::id())
                                                        <a
                                                            class="edit-button btn d-flex align-items-center edit"><span
                                                                class="material-symbols-rounded">edit</span></a>
                                                    @endif
                                                    @if($reaction->user->id === Auth::id() || (auth()->user()->roles->contains('role', 'Zeeverkenners Leiding') || auth()->user()->roles->contains('role', 'Administratie') || auth()->user()->roles->contains('role', 'Bestuur')|| auth()->user()->roles->contains('role', 'Ouderraad')))
                                                        <a class="delete-button btn d-flex align-items-center"
                                                           data-id="{{ $post->id }}"
                                                           ...
                                                           data-link="{{ route('zeeverkenners.comment.delete', ['id' => $reaction->id, 'postId' => $post->id]) }}">
                                                            <span class="material-symbols-rounded">delete</span>
                                                        </a>
