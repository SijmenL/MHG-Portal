    ...
    <div class="container col-md-11">
        ...
        <div class="mt-1 bg-white p-2 rounded d-flex flex-row justify-content-between">
            ...
            <div class="d-flex flex-row">
                ...
                @endif
                @if((auth()->user()->roles->contains('role', '	Afterloodsen Organisator') || auth()->user()->roles->contains('role', 'Administratie') || auth()->user()->roles->contains('role', 'Bestuur')|| auth()->user()->roles->contains('role', 'Ouderraad')))
                    <a class="delete-button btn d-flex align-items-center"
                       ...
                       @endif
                       data-link="{{ route('afterloodsen.post.delete', $post->id) }}"><span
        ...
        <div id="comments" class="p-3 comment-section">
            ...
            <div class="bg-light rounded-2 p-3">
                <div class="container">
                    <form method="POST" action="{{ route('afterloodsen.comment-post', $post->id) }}">
                        @csrf
                                ...
                                <div class="content mt-1">
                                    ...
                                    <div class="forum-buttons">
                                        ...
                                        @if($comment->user->id === Auth::id() || (auth()->user()->roles->contains('role', 'Afterloodsen Organisator') || auth()->user()->roles->contains('role', 'Administratie') || auth()->user()->roles->contains('role', 'Bestuur')|| auth()->user()->roles->contains('role', 'Ouderraad')))
                                            <a class="delete-button btn d-flex align-items-center"
                                               ...
                                               data-link="{{ route('afterloodsen.comment.delete', ['id' => $comment->id, 'postId' => $post->id]) }}">
                                    ...
                                    <form method="POST" class="comment-form" data-post-id="{{ $post->id }}"
                                        ...
                                        <div class="d-flex flex-row gap-2 align-items-end comment-input">
                                            <div class="text-input border border-1 border-info rounded w-100" id="text-input" style="min-height: 75px"
                                                ...
                                                <div class="forum-buttons">
                                                    ...
                                                    @if($reaction->user->id === Auth::id() || (auth()->user()->roles->contains('role', 'Afterloodsen Organisator') || auth()->user()->roles->contains('role', 'Administratie') || auth()->user()->roles->contains('role', 'Bestuur')|| auth()->user()->roles->contains('role', 'Ouderraad')))
                                                        <a class="delete-button btn d-flex align-items-center"
                                                           ...
                                                           data-link="{{ route('afterloodsen.comment.delete', ['id' => $reaction->id, 'postId' => $post->id]) }}">
                                                            <span class="material-symbols-rounded">delete</span>
                                                        </a>
