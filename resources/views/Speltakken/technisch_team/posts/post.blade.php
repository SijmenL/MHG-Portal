    ...
    <div class="container col-md-11">
        ...
        <div class="mt-1 bg-white p-2 rounded d-flex flex-row justify-content-between">
            ...
            <div class="d-flex flex-row">
                ...
                @endif
                @if((auth()->user()->roles->contains('role', 'Hoofd Technisch Team') || auth()->user()->roles->contains('role', 'Administratie') || auth()->user()->roles->contains('role', 'Bestuur')|| auth()->user()->roles->contains('role', 'Ouderraad')))
                    <a class="delete-button btn d-flex align-items-center"
                       ...
                       @endif
                       data-link="{{ route('technisch_team.post.delete', $post->id) }}"><span
        ...
        <div id="comments" class="p-3 comment-section">
            ...
            <div class="bg-light rounded-2 p-3">
                <div class="container">
                    <form method="POST" action="{{ route('technisch_team.comment-post', $post->id) }}">
                        @csrf
                            ...
                            <div class="w-100 p-1" style=" width: 100%; overflow: auto">
                                ...
                                <div class="content mt-1">
                                        ...
                                        @if($comment->user->id == Auth::id())
                                            <a
                                                class="edit-button btn d-flex align-items-center edit"><span
                                                    class="material-symbols-rounded">edit</span></a>
                                        @endif
                                        @if($comment->user->id == Auth::id() || (auth()->user()->roles->contains('role', 'Hoofd Technisch Team') || auth()->user()->roles->contains('role', 'Administratie') || auth()->user()->roles->contains('role', 'Bestuur')|| auth()->user()->roles->contains('role', 'Ouderraad')))
                                            <a class="delete-button btn d-flex align-items-center"
                                               data-id="{{ $post->id }}"
                                               ...
                                               data-link="{{ route('technisch_team.comment.delete', ['id' => $comment->id, 'postId' => $post->id]) }}">
                                    ...
                                    <form method="POST" class="comment-form" data-post-id="{{ $post->id }}"
                                          ...
                                          action="{{ route('technisch_team.reaction-post', ['id' => $post->id, 'commentId' => $comment->id]) }}">
                                        @csrf
                                                    ...
                                                    @if($reaction->user->id == Auth::id())
                                                        <a
                                                            class="edit-button btn d-flex align-items-center edit"><span
                                                                class="material-symbols-rounded">edit</span></a>
                                                    @endif
                                                    @if($reaction->user->id == Auth::id() || (auth()->user()->roles->contains('role', 'Hoofd Technisch Team') || auth()->user()->roles->contains('role', 'Administratie') || auth()->user()->roles->contains('role', 'Bestuur')|| auth()->user()->roles->contains('role', 'Ouderraad')))
                                                        <a class="delete-button btn d-flex align-items-center"
                                                           data-id="{{ $post->id }}"
                                                           ...
                                                           data-link="{{ route('technisch_team.comment.delete', ['id' => $reaction->id, 'postId' => $post->id]) }}">
                                                            <span class="material-symbols-rounded">delete</span>
                                                        </a>
