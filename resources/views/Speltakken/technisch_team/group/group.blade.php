...
@section('content')
    <div id="popUp" class="popup d-none" style="margin-top: -122px">
        <div class="popup-body">
            ...
            <p>Alle technisch team leden die aan je zoekopdracht voldoen worden geëxporteerd.</p>
            <div class="bg-light rounded-2 p-3">
                <h2>Opties</h2>
                <div class="quick-action-bar">
                    <form class="m-0 p-0 quick-action" action="{{ route('technisch_team.groep.export') }}" method="POST">
                        @csrf
    ...
    <div class="container col-md-11">
                            ...
                            @if(auth()->user() && (auth()->user()->roles->contains('role', 'Hoofd Technisch Team') || auth()->user()->roles->contains('role', 'Administratie') || auth()->user()->roles->contains('role', 'Bestuur')))
                                <th>
                                    <div class="d-flex flex-row flex-wrap gap-2">
                                        <a href="{{ route('technisch_team.groep.details', ['id' => $all_user->id]) }}"
        ...
        @else
            <div class="alert alert-warning d-flex align-items-center" role="alert">
                <span class="material-symbols-rounded me-2">person_off</span>Geen technisch team leden gevonden...
            </div>
        @endif
    </div>
@endsection
