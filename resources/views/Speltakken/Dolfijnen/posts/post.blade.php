                                ...
                                <div class="comment p-3 rounded bg-white" data-comment-id="{{ $reaction->id }}"
                                    ...
                                    <div class="d-flex flex-row gap-2">
                                        ...
                                        <div class="w-100 p-1" style=" width: 100%; overflow: auto">
                                            <div class="d-flex flex-row-responsive" style="margin-bottom: -15px;">
                                                <p><span
                                                        class="fw-bold @if($post->user_id === $reaction->user_id) post-op @endif">@if($reaction->user->roles->contains('role', 'Dolfijnen Leiding'))
                                                            {{ $reaction->user->dolfijnen_name }}
                                                        @else
                                                    ...
                                                    @if($reaction->user->id === Auth::id())
                                                        <a
                                                            class="edit-button btn d-flex align-items-center edit"><span
                                                                class="material-symbols-rounded">edit</span></a>
                                                    @endif
                                                    @if($reaction->user->id === Auth::id() || (auth()->user()->roles->contains('role', 'Dolfijnen Leiding') || auth()->user()->roles->contains('role', 'Administratie') || auth()->user()->roles->contains('role', 'Bestuur')|| auth()->user()->roles->contains('role', 'Ouderraad')))
                                                        <a class="delete-button btn d-flex align-items-center"
                                                           data-id="{{ $post->id }}"
                                                           ...
                                                           data-link="{{ route('dolfijnen.comment.delete', ['id' => $reaction->id, 'postId' => $post->id]) }}">
                                                            <span class="material-symbols-rounded">delete</span>
                                                        </a>
