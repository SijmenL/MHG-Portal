        ...
        @if(isset($lesson))
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('agenda.activity', ['lessonId' => $lesson->id, 'id' => $activity->id, 'view' => $view, 'month' => $monthOffset, 'all' => $wantViewAll ? 1 : 0]) }}">{{ $activity->title }}</a></li>
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a
                        ...
                        agenda</a></li>
                <li class="breadcrumb-item"><a href="{{ route('agenda.activity', ['id' => $activity->id, 'view' => $view, 'month' => $monthOffset, 'all' => $wantViewAll ? 1 : 0]) }}">{{ $activity->title }}</a></li>
                <li class="breadcrumb-item active" aria-current="page">Bewerken</li>
            </ol>
        </nav>
        ...
        <div class="d-flex align-items-center gap-2 w-100 mt-2">
            ...
            <strong>{{ $message }}</strong>
                    ...
                    <div class="d-flex align-items-center flex-row mt-3 gap-2">
                        ...
                        @if(isset($lesson))
                            <a class="btn btn-danger text-white" href="{{route('agenda.activity', ['id' => $activity->id, 'lessonId' => $lesson->id, 'view' => $view, 'month' => $monthOffset, 'all' => $wantViewAll ? 1 : 0]) }}">Annuleren</a>
                        ...
                        @else
                        <a class="btn btn-danger text-white" href="{{ route('agenda.activity', ['id' => $activity->id, 'view' => $view, 'month' => $monthOffset, 'all' => $wantViewAll ? 1 : 0]) }}">Annuleren</a>
                        <a class="delete-button btn btn-outline-danger"
                           data-id="{{ $activity->id }}"
                           data-name="{{ $activity->title }}"
